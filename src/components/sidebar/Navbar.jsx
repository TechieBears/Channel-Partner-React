                ...
                <div className="flex item-center">
                    ...
                    <div className="flex items-center" onClick={() => setCard(!card)}>
                        <div className="flex-shrink-0">
                            <img className="object-cover w-10 h-10 rounded-full" src={user?.loggedUserDetails?.profile_pic != "" ? user.loggedUserDetails.profile_pic : userImg} alt="user" />
                        </div>
            ...
            <div className={`${card ? "-top-96 opacity-0" : "top-20 opacity-100"} bg-white  transition-all ease-in-out duration-700 fixed shadow-sm border right-6 py-2 rounded-lg z-20 px-4 lg:px-6 lg:py-1`}>
                <div className="flex items-center pb-3 border-b border-slate-200 lg:hidden">
                    ...
                    <div className="ml-3 space-y-1">
                        <h4 className="text-sm font-semibold leading-none md:text-base font-tb text-slate-800">{user?.loggedUserDetails?.first_name + " " + user?.loggedUserDetails?.last_name}</h4>
                        <h4 className="text-sm font-medium leading-none text-gray-400 capitalize md:text-base">{user?.loggedUserDetails?.role}</h4>
                    </div>
                </div>
