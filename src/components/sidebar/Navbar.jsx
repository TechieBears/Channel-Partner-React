        ...
        <div className=" flex justify-between items-center py-5  px-5 sm:px-6 pt-24 sm:pt-4 pb-0 sm:pb-5">
            <div className="min-w-0 flex-1">
                <h2 className="font-tb font-bold text-2xl  md:text-2xl lg:text-2xl whitespace-nowrap ">
                    {greetingTime(new Date())},<span className="capitalize text-sky-400">{user?.loggedUserDetails?.first_name}</span>
                </h2>
            ...
            <div className={`${card ? "-top-96 opacity-0" : "top-20 opacity-100"} bg-white  transition-all ease-in-out duration-700 fixed shadow-sm border right-6 py-2 rounded-lg z-20 px-4 lg:px-6 lg:py-1`}>
                <div className="flex items-center border-b border-slate-200 pb-3 lg:hidden">
                    ...
                    <div className="ml-3 space-y-1">
                        <h4 className="text-sm md:text-base font-tb font-semibold leading-none text-slate-800">{user?.loggedUserDetails?.first_name + " " + user?.loggedUserDetails?.last_name}</h4>
                        <h4 className="text-sm md:text-base font-medium leading-none capitalize text-gray-400">{user?.loggedUserDetails?.role}</h4>
                    </div>
                </div>
                <div className="p-2 pb-0">
                    <NavLink to={"/profile"} onClick={() => setCard(!card)} className="flex items-center space-x-2 pb-2 group transition-all duration-700">
                        ...
                        <h4 className='font-tbPop text-sm lg:text-lg md:text-base font-semibold group-hover:text-sky-400 text-slate-700 capitalize'>Profile</h4>
                    </NavLink>
                    <NavLink className="flex items-center space-x-2 pb-2 group transition-all duration-700">
                        ...
                        <h4 className='font-tbPop text-sm lg:text-lg md:text-base font-semibold group-hover:text-sky-400 text-slate-700 capitalize'>Setting</h4>
                    </NavLink>
