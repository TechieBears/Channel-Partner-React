        ...
        {subMenu ?
            <NavLink to={link} onClick={() => { handleclick(index), setTextIndex(index) }} className={`${!isActiveLink ? "px-7 justify-between" : "justify-center"}  flex items-center  space-x-2  group pt-2 pb-1  my-1 w-full origin-left relative transition-all duration-500 `}>
                ...
                <div className="flex items-center  space-x-2">
                    <span className="text-slate-800  group-hover:text-[#F97316] duration-500 transition-all origin-left icon-wrapper">{icon}</span>
                    {!isActiveLink && <h4 className="text-base font-tb font-bold origin-left text-slate-800 group-hover:text-[#F97316] duration-500 whitespace-nowrap transition-all  text-title">{title}</h4>}
                </div>
